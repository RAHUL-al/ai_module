...
async def audio_ws(websocket: WebSocket):
    ...
    config = {
        'silence_threshold': 2.0,  # Time to wait after speech ends
        'min_utterance_length': 3,  # Minimum words to consider as valid speech
        'max_chunk_duration': 10.0,  # Maximum time to buffer speech before processing
        'blocklist': [
        ...
        while True:
            ...
            if message["type"] == "websocket.receive" and "bytes" in message:
                if session_state['processing_active']:
                    continue  # Skip if we're currently processing
                ...
                if current_silence > 0.2:  # Silence detected in this chunk
                    ...
                    # Check if we should process (silence threshold reached)
                    ...
                    # Check if we've hit max buffer time
                    buffer_duration = len(session_state['audio_buffer']) / 1000  # Convert ms to seconds
        ...
        with aiohttp.ClientSession() as session:
            with session.get(api_endpoint) as response:
                if response.status == 200:
                    ...
                    logging.warning(f"[SCRAPING] Failed with status {response.status}: {response.text()}")
    except Exception as e:
